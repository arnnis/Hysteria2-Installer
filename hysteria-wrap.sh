...
install_required_packages() {
    REQUIRED_PACKAGES=("curl" "openssl" "lsb-release" "gpg")
...
install_warp() {
    if dpkg -l | grep -q cloudflare-warp; then
        echo "Cloudflare WARP is already installed. Configuring..."
    ...
    echo "Cloudflare WARP installed and configured in proxy mode."
}
...
if [ -d "/root/hysteria" ]; then
    ...
    case $choice in
        1)
            # Reinstall
            # Ask about WARP before removing
            echo ""
            echo "Do you want to route traffic through Cloudflare WARP? (y/n)"
            read -p "" use_warp
            if [[ $use_warp == [Yy] ]]; then
                with_warp=true
            else
...
http:
  listen: 127.0.0.1:10809"
    ...
    # For new installation, ask about WARP
    echo ""
    echo "Do you want to route traffic through Cloudflare WARP? (y/n)"
    read -p "" use_warp
    if [[ $use_warp == [Yy] ]]; then
...
# Add outbounds and acl if using WARP
if [ "$with_warp" = true ]; then
